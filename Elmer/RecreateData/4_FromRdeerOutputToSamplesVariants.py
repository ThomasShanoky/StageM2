...
cutoff_range = list(range(1, 31))

score = {c:0 for c in cutoff_range}

for cutoff in cutoff_range:
    for gene in Genes:

        Gene_variants_file = f"{directory}/Full_AML_GeneVar/Full_AML_{gene}_variants.csv"
        ...
        for index, row in Gene_variants.iterrows():
            ...
            if in_cosmic == 1: #on ne prend que les variants dont on est sûr qu'ils sont oncogéniques
                ...
                file_alt1 = f"{directory}/RdeerOutput/kmer_to_search_{gene}_var{i}_output_rdeer_part1_alt.tsv"
                file_alt2 = f"{directory}/RdeerOutput/kmer_to_search_{gene}_var{i}_output_rdeer_part2_alt.tsv"
                if os.path.exists(file_alt1) and os.path.exists(file_alt2):
                    ...
                    file_ref1 = f"{directory}/RdeerRefOutput/kmer_to_search_{gene}_ref{i}_output_rdeer_part1_ref.tsv"
                    file_ref2 = f"{directory}/RdeerRefOutput/kmer_to_search_{gene}_ref{i}_output_rdeer_part2_ref.tsv"
                    ...
                    for column in df_alt.columns:
                        ...
                        if sum_count_kmer_alt >= cutoff*nb_kmers_alt:
                            n += 1
                            if os.path.exists(file_ref1) and os.path.exists(file_ref2):
                    ...
                    ResTheorEtPrat.append((698*total_freq, n))

            i += 1

        # TableMutants["sampleID"] = TableMutants["sampleID"].str[:6]
        # TableMutants.to_csv(f"{directory}/newMUTdata/{gene}_alt_perso.csv", index=False)

        # print(f"{gene}")
        # print(ResTheorEtPrat)
        score_gene = 0
        for tup in ResTheorEtPrat:
            score_gene += abs(tup[0] - tup[1])
        score[cutoff] += score_gene
        # print(f"score : {score_gene}")
